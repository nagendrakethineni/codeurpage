...
<head>  
    ...
    <script src='js/table-sortable.min.js'></script>
          ...
          <div class="card shadow p-1">
              <input type='search' id='insearch' class='form-control w-25 float-left mb-1' placeholder='Search'>
              <div id='inpticketstable' class="ticketstable"></div>
              <div class='pages'></div>
          </div>    
            <script>
            var data2 = [
            <?php   
                  $sqlnew = "select reqid1,email,resume,product,plan,status,updted,updatedby,pending from cg where status='INPROGRESS' UNION ALL select reqid1,email,course,product,plan,status,updted,updatedby,pending from ot WHERE STATUS='INPROGRESS' UNION ALL select reqid1,email,typereq,product,plan,status,updted,updatedby,pending from web_dev where STATUS='INPROGRESS' UNION ALL select reqid1,email,domainreq,product,plan,status,updted,updatedby,pending from web_hos where STATUS='INPROGRESS'UNION ALL select reqid1,email,typereq,product,plan,status,updted,updatedby,pending from web_des where STATUS='INPROGRESS' UNION ALL select reqid1,email,typereq,product,plan,status,updted,updatedby,pending from app_dev where STATUS='INPROGRESS'UNION ALL select reqid1,email,tagline,product,plan,status,updted,updatedby,pending from l_des where STATUS='INPROGRESS'";         
            ...
            var table2 = $('#inpticketstable').tableSortable({
                data: TestData2.data,
                columns: TestData2.columns,
