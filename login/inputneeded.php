...
<head>  
    ...
    <script src='js/table-sortable.min.js'></script>
          ...
          <div class="card shadow p-1">
              <input type='search' id='innsearch' class='form-control w-25 float-left mb-1' placeholder='Search'>
              <div id='inpnticketstable' class="ticketstable"></div>
              <div class='pages'></div>
          </div>    
            <script>
            var data3 = [
            <?php   
                  $sqlnew = "select reqid1,email,resume,product,plan,status,updted,updatedby,pending from cg where status='INPUTNEEDED' UNION ALL select reqid1,email,course,product,plan,status,updted,updatedby,pending from ot WHERE STATUS='INPUTNEEDED' UNION ALL select reqid1,email,typereq,product,plan,status,updted,updatedby,pending from web_dev where STATUS='INPUTNEEDED'UNION ALL select reqid1,email,typereq,product,plan,status,updted,updatedby,pending from web_des where STATUS='INPUTNEEDED' UNION ALL select reqid1,email,typereq,product,plan,status,updted,updatedby,pending from app_dev where STATUS='INPUTNEEDED' UNION ALL select reqid1,email,tagline,product,plan,status,updted,updatedby,pending from l_des where STATUS='INPUTNEEDED'";         
            ...
            var table3 = $('#inpnticketstable').tableSortable({
                data: TestData3.data,
                columns: TestData3.columns,
